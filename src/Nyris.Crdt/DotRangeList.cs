...
namespace Nyris.Crdt
{
    ...
    public sealed class DotRangeList
    {
        ...
        public void Merge(DotRange range)
        {
            _lock.EnterWriteLock();
            try
            {
        ...
        public ulong GetLastBeforeUnknown()
        {
            _lock.EnterReadLock();
            try
            {
                if (_ranges.Count == 0 || _ranges[0].From > 1) return 0;
                return _ranges[0].To - 1;
            }
            finally
            {
                _lock.ExitReadLock();
        ...
        public ulong GetNew()
        {
            ...
            try
            {
                if (_ranges.Count == 0)
                {
                    _ranges.Add(new DotRange(1, 2));
                    return 1;
                }

                var lastRange = _ranges[^1];
                var nextDot = lastRange.To;
                _ranges[^1] = new DotRange(lastRange.From, nextDot + 1);
                return nextDot;
            }
            finally
            {
        ...
        private void MergeInternal(DotRange range)
        {
            if (_ranges.Count == 0)
            {
                _ranges.Add(range);
            ...
            if (i + 1 == _ranges.Count)
            {
                ...
                _ranges[i] = new DotRange(_ranges[i].From, Math.Max(range.To, _ranges[i].To));
                return;
            }
            ...
            if (i < 0)
            {
                ...
                {
                    _ranges[0] = new DotRange(range.From, first.To);
                    return;
                }
            ...
            if (_ranges[i].To < range.From)
            {
                ...
                if (j == _ranges.Count)
                { 
                    _ranges[i + 1] = new DotRange(range.From, range.To);
                ...
                if (range.To >= _ranges[j].From)
                {
                    ...
                    _ranges[i + 1] = new DotRange(range.From, _ranges[j].To);
            ...
            if (k == _ranges.Count)
            { 
                _ranges[i] = new DotRange(_ranges[i].From, range.To);
            ...
            if (range.To >= _ranges[k].From)
            {
                ...
                _ranges[i] = new DotRange(_ranges[i].From, _ranges[k].To);
        ...
        private void MergeInternal(ulong dot)
        {
            if (_ranges.Count == 0)
            {
                _ranges.Add(new DotRange(dot, dot + 1));
                return;
            }
            ...
            if(i < 0)
            {
                ...
                if (first.From > dot + 1)
                {
                    _ranges.Insert(0, new DotRange(dot, dot + 1));
                }
                else
                {
                    _ranges[0] = new DotRange(dot, first.To);
                }
            ...
            if (prevRange.To > dot) return; // Nothing to update, dot was already accounted for.
            ...
            if (i + 1 == _ranges.Count)
            {
                if (prevRange.To == dot)
                {
                    _ranges[i] = new DotRange(prevRange.From, dot + 1);
                }
                else
                {
                    _ranges.Add(new DotRange(dot, dot + 1));
            ...
            if (prevRange.To == dot)  // If new dot is coming right after previous range, we are merging it with it
            {
                ...
                {
                    _ranges[i] = new DotRange(prevRange.From, nextRange.To);
            ...
            else // Dot is not right after previous range...
            {
                ...
                if (dot + 1 == nextRange.From) {
                    _ranges[i + 1] = new DotRange(dot, nextRange.To);
                    return;
                }
                
                // otherwise - simply insert new 1-length range in the appropriate spot 
                _ranges.Insert(i + 1, new DotRange(dot, dot + 1));
            } 
        }
