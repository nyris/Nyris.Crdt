...
public class ObservedRemoveCoreDeltaDtoFormatter<TActorId, TValue> 
    : IMessagePackFormatter<OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDto> 
    ...
    where TValue : IEquatable<TValue>
{
    ...
    public void Serialize(ref MessagePackWriter writer, OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDto value, MessagePackSerializerOptions options)
    {
        var actorFormatter = options.Resolver.GetFormatterWithVerify<TActorId>();
        switch (value)
        {
            case OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDtoAddition (var val, var actorId, var version):
                ...
                break;
            case OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDtoDeletedDot (var actorId, var version):
                writer.WriteUInt8(RemovalDelta);
                actorFormatter.Serialize(ref writer, actorId, options);
                writer.WriteUInt64(version);
                break;
            case OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDtoDeletedRange (var actorId, var range):
    ...
    public OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDto Deserialize(ref MessagePackReader reader, MessagePackSerializerOptions options)
    {
        try
        {
            var actorFormatter = options.Resolver.GetFormatterWithVerify<TActorId>();
            ...
            switch (typeByte)
            {
                case AdditionDelta:
                    ...
                    return OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDto.Added(value, actorId, version);
                case RemovalDelta:
                    actorId = actorFormatter.Deserialize(ref reader, options);
                    version = reader.ReadUInt64();
                    return OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDto.Removed(actorId, version);
                case RemovalRangeDelta:
                    actorId = actorFormatter.Deserialize(ref reader, options);
                    var range = new Range(reader.ReadUInt64(), reader.ReadUInt64());
                    return OptimizedObservedRemoveCore<TActorId, TValue>.DeltaDto.Removed(actorId, range);
                default:
